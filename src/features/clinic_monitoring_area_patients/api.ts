    ...
    public async get_records_list({options: {filters}, client = db}: method_payload<get_records_payload>) {
        const sql = `
            SELECT u.id,
                   ...
                   u.blood_type,
                   u.rh,
                   (SELECT row_to_json(alerts) from alerts where alerts.patient_id = u.id ORDER BY alerts.created_at DESC LIMIT 1) as last_alert
...
export const clinic_monitoring_area_patients_api = new Api({
    table_name: 'clinic_monitoring_area_patients'
});
